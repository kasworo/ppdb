<?php
	...
	$objPHPExcel->getProperties()->setCreator("Kasworo Wardani")
		...
		->setSubject("Office 2007 XLSX Test Document")	->setDescription("Soal Export")	->setKeywords("office 2007 openxml php")	->setCategory("Daftar Siswa");
	...
	$no=0;
	$baris=5;
	$cek=cekdata("v_calsis");
	if($cek>0){
		$dcs=viewdata("v_calsis",'','',"nama ASC");
		foreach($dcs as $cs)
		{
			$no++;
			$baris++;
    ...
    ->getStyle('A1:'. $objPHPExcel->getActiveSheet()->getHighestColumn().$objPHPExcel->getActiveSheet()->getHighestRow())->getNumberFormat()
